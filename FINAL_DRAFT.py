...
def tutorialMenu():
    ...
    tutorialPicture = pygame.transform.scale(tutorialPicture, (round((WIDTH)), round((HEIGHT))))
    while True:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                quitgame()
        ...
        screen.fill(WHITE)
        screen.blit(tutorialPicture, (0, 0))
        button("Menu", (WIDTH*0.5)-125, HEIGHT * 0.8, 250, 50, RED, BRIGHT_RED, mainMenu)
